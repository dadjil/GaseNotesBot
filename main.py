...
def localization_and_num(call):
    ...
    bot.send_message(call.message.chat.id, text=temporal_local.get_reply_text("before_start"))

    auto_numeration_markup = telebot.types.InlineKeyboardMarkup()

    yes = telebot.types.InlineKeyboardButton(text=temporal_local.get_list_of_inline_buttons()[0],
                                             callback_data='num_yes')
    no = telebot.types.InlineKeyboardButton(text=temporal_local.get_list_of_inline_buttons()[1],
                                            callback_data='num_no')
    auto_numeration_markup.add(yes, no)
    bot.send_message(call.message.chat.id, text=temporal_local.get_inline_menu_text("num"),
                     reply_markup=auto_numeration_markup)


@bot.callback_query_handler(func=lambda call: call.data.startswith('num_'))
def numeration_and_date(call):
    ...
    bot.send_message(call.message.chat.id, text=temporal_local.get_inline_menu_text("date"),
